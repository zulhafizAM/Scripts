...
@inject()
export default class MedicalAnnual_employee_claimServices {
    constructor(protected commonService: CommonServices) {}
    async getAnnual_employee_claims(payload: Record<string, any>) {
        try {
            const query = await Annual_employee_claim.query()
            let response = {
                meta: {
