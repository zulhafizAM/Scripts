...
export default class ActingMainServices {
    public async getMains(
        payload,
    ): Promise<DefaultListResponse<ActingMainsResponse>> {
        let mains = new DefaultListResponse<ActingMainsResponse>();
        ...
        mains = {
            ...
            dataList: mainList.map((result) => ({
                id: Number(result.id),            })),
        } as DefaultListResponse<ActingMainsResponse>;
