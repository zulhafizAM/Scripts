...
export default class SelfTransfers extends BaseModel {
    ...
    public static withPreloadedList = scope<typeof SelfTransfers>((query) => {
        query
            .preload('employee', (query) => query.preload('personalDetail'))
            .preload('selfTransferProcess', (query) =>
                query.preload('appointedApprover', (query) =>
                    query.preload('personalDetail'),
                ),
            );
    });
    ...
    public selfTransferReason: HasMany<typeof SelfTransferReason>;

    @beforeFind()
    public static async preloadPlacement(query) {
        query.preload('firstChoicePlacement');

        query.preload('secondChoicePlacement');

        query.preload('employee', (query) =>
            query
                .preload('unit')
                .preload('position')
                .preload('grade')
