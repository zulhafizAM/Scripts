...
import ForcedTransferByDirector from './ForcedTransferByDirector';
import Employee from './Employee';
import { employeePreloads } from 'App/Helpers/preloads';

export default class ForcedByDirectorProcess extends BaseModel {
    public static table = 'forcedByDirectorProcess';
    public static primaryKey = 'id';
    public static incrementing = false;

    public static namingStrategy = new CamelCaseNamingStrategy();

    public static withPreloadedList = scope<typeof ForcedByDirectorProcess>((query) => {
        query
            .preload('force')
    ...
    @belongsTo(() => ForcedTransferByDirector, {
        foreignKey: 'forceId',
        onQuery: (query) => {
        query.where('active', true);
        },
    })
    public force: BelongsTo<typeof ForcedTransferByDirector>;
