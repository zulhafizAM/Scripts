...
export default class extends BaseSeeder {
	...
	public async run () {
		const accruedAnnualLeaves = await AccruedAnnualLeave.all();
		const employees = await Employee.all();
		const faker = new Faker({ locale: [en] });
		await AccruedAnnualLeaveProcess.createMany([
			{
				accruedLeaveId: accruedAnnualLeaves![Math.floor(Math.random() * 10)].id,
