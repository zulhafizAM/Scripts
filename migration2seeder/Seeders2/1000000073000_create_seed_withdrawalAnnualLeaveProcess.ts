...
export default class extends BaseSeeder {
	...
	public async run () {
		const withdrawalAnnualLeaves = await WithdrawalAnnualLeave.all();
		const employees = await Employee.all();
		const faker = new Faker({ locale: [en] });
		await WithdrawalAnnualLeaveProcess.createMany([
			{
				withdrawalLeaveId: withdrawalAnnualLeaves![Math.floor(Math.random() * 10)].id,
